...
export default function PayrollDetailPageContent() {
  const router = useRouter();
  const params = useParams();
  const { toast } = useToast();
  const payrollId = typeof params.id === 'string' ? params.id : '';
  ...
  const handleDownloadSlip = async () => {
    if (!payrollId) return;
    try {
      const response = await apiClient.get(`/payroll/${payrollId}/slip`, {
        responseType: 'blob',
      });
